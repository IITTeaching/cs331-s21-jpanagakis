...
class ArrayList:
    ...
    def append(self, value):
        ...
        ### BEGIN SOLUTION
        newData = ConstrainedList(self.len+1)
    ...
    def insert(self, idx, value):
        ...
        nidx = self._normalize_idx(idx)
        self.len += 1
        if nidx > self.len: raise IndexError
        ...
        for i in range(0,self.len-1):
            if i == nidx:
                newData[count] = value
                count += 1
            newData[count] = self.data[i]
            count += 1
        
        if nidx == self.len-1: newData[nidx] = value
        self.data = newData
        ### END SOLUTION
    def pop(self, idx=-1):
    ...
    def remove(self, value):
        ...
        count = 0
        for i in range(0,self.len):
            if self.data[i] == value and not found:
                count += 1
                found = True
            newData[i] = self.data[count]
            count += 1
        self.data = newData
        ### END SOLUTION
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        newList = ArrayList(self.len + len(other))
        newList.data = ConstrainedList(self.len + len(other))
        
        if len(self.data) == 0 and len(other) == 0:
            return newList
        
        else:
            for i in range(0,self.len):
                newList.data[i] = self.data[i]
            for i in range(0,len(other)):
                newList.data[i+self.len] = other[i]
            self.len += len(other)
            return newList
        ### END SOLUTION
