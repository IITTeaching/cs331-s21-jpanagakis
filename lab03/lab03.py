...
class PrefixSearcher():
    ...
    def __init__(self, document, k):
        ...
        for i in range(0, len(document)):
            ...
            cmp = lambda x,y:  0 if x in y else (-1 if x < y else 1)
    ...
    def search(self, q):
        ...
        strcmp = lambda x,y:  0 if y in x else (-1 if x < y else 1)
        if mybinsearch(self.lst, q, strcmp) < 0:
            return False
        else:
            return True
# 30 Points
...
class SuffixArray():
    ...
    def positions(self, searchstr: str):
        ...
        """
        strcmp = lambda x,y:  0 if self.lst[x][:len(searchstr)] == y else (-1 if self.lst[x] < y else 1)
